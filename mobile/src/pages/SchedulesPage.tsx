  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <div className="bg-slate-800 text-white rounded-2xl p-5">
        <p className="text-xs font-bold text-blue-300 uppercase tracking-wider mb-1">Próximo</p>
      ...
      <div className="bg-white rounded-2xl shadow p-5 flex items-center justify-between gap-4">
        ...
        <button
          ...
          className={`w-12 h-6 rounded-full transition-colors relative shrink-0 ${push01 ? 'bg-green-500' : 'bg-slate-200'}`}
        >
          <span
      ...
      {/* Lista */}
      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider px-1">Horarios de recolección</p>
      <div className="space-y-2">
        {schedules.map((s) => (
          <div key={s.id} className="bg-white rounded-2xl shadow px-5 py-4 flex justify-between items-center">
            <span className="font-semibold text-slate-800">🗓 {s.day_name}</span>
            <div className="text-right">
              <p className="text-lg font-black text-green-600">{s.time}</p>
